...
from utils import create_summary_listings_table
from loader_factory import LoaderFactory

def extract_transform_summary_listings():
    ...
    target_table = os.getenv("SUMMARY_LISTINGS_STG_TABLE_NAME")

    stg_clean_summary_listing_data_path = os.getenv("STG_CLEANSED_DATA_PATH")
    summary_listing_file_path = os.path.join(stg_clean_summary_listing_data_path, f"{target_table}.csv")
    if os.path.exists(summary_listing_file_path):
        os.remove(summary_listing_file_path)
    ...
    if target_table not in inspector.get_table_names(schema=target_schema):
        create_summary_listings_table(target_db_connection_string, target_schema, target_table)
        ...
        for country in countries:
            ...
            for province in provinces:
                ...
                for city in cities:
                    ...
                    #Load to target
                    file_exists = os.path.exists(summary_listing_file_path)
                    df.to_csv(summary_listing_file_path, mode='a', index=False, header=not file_exists)
...
with DAG(
    "stg_etl_airbnb_summary_listings",
    default_args=default_args,
    schedule_interval="@daily",
    catchup=False,
) as dag:

    extract_transform_listings = PythonOperator(task_id="stg_summary_listing_extract_transform", python_callable=extract_transform_summary_listings)
